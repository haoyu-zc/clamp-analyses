...
Inputs the ARCHS4 families need that this integration does not produce:
**`[MARC: ARCHS4 raw h5 + preprocess outputs (`00_preprocess/metadata_filtered.rds`,
`fbm_filtered.bk`), the coverage cells' `fbm_subsampled.bk` (written by
`subsample_cells.R` but undeclared), `scripts/archs4/clamp.R` referenced by
`clampbase_bp_coverage_cell`, the MSigDB gene-set files under `data/pathways/`
(`go_bp.Hs.symbols.gmt` for the CLAMPfull_bp prior, `c2.cp.*`, `c5.go.cc.*`,
`c8.all.*`, `h.all.*`, `Cell_marker_Human.xlsx`) — where they come from and how to
stage them.]`**
